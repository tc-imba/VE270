...
\begin{tikzpicture}[>/.tip={Latex}]
	...
	\draw[->] (on2.north) [bend right=22.5] edge node [above] {$X'$} (wait.south);
...
\section{}
Encode the states: Wait: 00, On1: 01, On2: 10, End: 03.


\begin{center}
\begin{tikzpicture}
	%\draw (0,0) node [draw,shape=rectangle,text width=2cm,align=center] {2-bit state register};
	\drawregister{(0,0)}{r}{2-bit state register}{2};
...
\begin{tikzpicture}[>/.tip={Latex}]
	...
	\draw[->] (C2) edge node [below] {$I'$} (C1);
	\draw[->] (C3) edge node [above right] {$I'$} (C1);
	\draw[->] (C4) edge node [right] {$I'$} (C1);
\end{tikzpicture}
\end{center}
