...
\begin{tikzpicture}[>/.tip={Latex}]
	...
	\node [left] at (C4.west) {$xyz=100$};
	\draw (-1,-1) node (W1) [draw,shape=circle,minimum size=1.25cm] {W1};
	\node [below] at (W1.south) {$xyz=000$};
	...
	\draw[->] (C1) edge [bend right=45] node [left] {$I'$} (W1);
	\draw[->] (W1) edge node [left] {$I$} (C1);
	\draw[->] (W2) edge [loop left] node [right] {$I'$} ();
	\draw[->] (W3) edge [in=30,out=60,loop] node [above right] {$I'$} ();
	\draw[->] (W1) edge [loop left] node [left] {$I'$} ();
